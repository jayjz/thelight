...
export interface Sql {
  ...
  /** Execute a callback on one database transaction/connection. */
...
function toSql(run: Run, transaction?: <T>(callback: (tx: Sql) => Promise<T>) => Promise<T>): Sql {
  ...
  sql.query = <T = Record<string, unknown>>(text: string, params: unknown[] = []) =>
    run<T>(text, params);
  sql.transaction = transaction ?? (async <T>(callback: (tx: Sql) => Promise<T>) => callback(sql));
  return sql;
}
...
function createNeonSql(): Promise<Sql> {
  globalRef.__pgSqlPromise__ ??= (async () => {
    ...
    return toSql(run, async <T>(callback: (tx: Sql) => Promise<T>) => {
      const client = await pool.connect();
      try {
        await client.query("BEGIN");
        const result = await callback(toSql(async <R>(text: string, params: unknown[]) => {
...
async function createPgliteSql(): Promise<Sql> {
  ...
  return toSql(run, async <T>(callback: (tx: Sql) => Promise<T>) => pg.transaction(async (tx) => callback(toSql(async <R>(text: string, params: unknown[]) => {
